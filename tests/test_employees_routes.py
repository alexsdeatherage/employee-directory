...
def test_list_employees_with_filters(client):
	...
	default_response = client.get("/employees")
	assert default_response.status_code == 200
	assert len(default_response.json()) == 2

	department_response = client.get("/employees", params={"department": "Engineering"})
	...
	status_response = client.get("/employees", params={"status": "inactive"})
